...
<script>
  ...
  export default {
    ...
    mounted () {
      if (this.$root.settings.role === 'provider' && this.appointment !== null) {
    ...
    methods: {
      resetSearchedCustomers () {
        this.filteredCustomersBookings = this.bookings.slice(0, this.customerSelectBoxLimit)
      },
        ...
        if (query !== '') {
          setTimeout(() => {
            let customersIds = this.options.entities.customers.filter(item => {
              return item.firstName.toLowerCase().indexOf(query.trim().toLowerCase()) > -1 ||
                item.lastName.toLowerCase().indexOf(query.trim().toLowerCase()) > -1 ||
                item.firstName.concat(' ').concat(item.lastName).toLowerCase().indexOf(query.trim().toLowerCase()) > -1 ||
                (item.email && item.email.toLowerCase().indexOf(query.trim().toLowerCase()) > -1)
            }).map(customer => customer.id).slice(0, this.customerSelectBoxLimit)

            let filteredCustomersBookings = this.bookings.filter(
              booking => customersIds.indexOf(booking.customer.id) !== -1
            )
